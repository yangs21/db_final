...
def nasdaq_year(conn):
    ...
    AND trade_date > date '1977-01-01' AND trade_date < date '1978-01-01'
...
def main():
    ...
    nasdaq = [r[1] for r in nasdaq_year(conn)]
    month = [r[0] for r in nasdaq_year(conn)]
    us_attack = [r[2] for r in count_US_attack(conn)]
    ...
    plt.plot(month, us_attack, label ='Numer of Attack in US in 1977', color='pink')
    plt.legend()
    ...
    plt.plot(month, nasdaq, label ='NASDAQ Volume Weighted Average Price (VWAP) in 1977', color='green')
    plt.legend()
